...
## run data prep modules so that their inits are run prior to sims.
LandRBiomass_preSim <- Cache(simInitAndSpades
...
saveSimList(LandRBiomass_preSim, file.path(simPaths$outputPath, paste0("preSimList_", runName)))

## SIMULATION WITH 10 REPS
## overwrite objects with those generated by the pre-sim
simObjects <- lapply(ls(LandRBiomass_preSim@.xData), FUN = function(x) {
  get(x, envir = LandRBiomass_preSim@.xData)
})
