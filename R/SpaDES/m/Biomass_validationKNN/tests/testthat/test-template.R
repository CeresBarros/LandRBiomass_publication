...
#    `functionName` is a function's name in your module (e.g., `Biomass_validationKNNEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ceres/GitHub/LandRBiomass_publication/R/SpaDES/m/Biomass_validationKNN/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("Biomass_validationKNN")
  path <- list(modulePath = "C:/Ceres/GitHub/LandRBiomass_publication/R/SpaDES/m",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    Biomass_validationKNN = list(.saveInitialTime = NA)
  )
  ...
  if (exists("Biomass_validationKNNEvent1", envir = .GlobalEnv)) {
    simOutput <- Biomass_validationKNNEvent1(mySim)
  } else {
  ...
  if (exists("Biomass_validationKNNEvent2", envir = .GlobalEnv)) {
    simOutput <- Biomass_validationKNNEvent2(mySim)
  } else {
